...
azr=`az monitor autoscale list -g $rgsource`
...
if [ "$count" -gt "0" ]; then
    ...
    for i in `seq 0 $count`; do
        ...
        trrg=`echo $azr | jq ".[(${i})].targetResourceUri" | cut -d'/' -f5 | tr -d '"'`
        trid=`echo $azr | jq ".[(${i})].targetResourceUri" | cut -d'/' -f9 | tr -d '"'`
                ...
                if [ "$kcount" -gt "0" ]; then
                    kcount=`expr $kcount - 1`
                        for k in `seq 0 $kcount`; do
                            ...
                            mtrrg=`echo $azr | jq ".[(${i})].profiles[(${j})].rules[(${k})].metricTrigger.metricResourceUri" | cut -d'/' -f5 | tr -d '"'`
                            mtrid=`echo $azr | jq ".[(${i})].profiles[(${j})].rules[(${k})].metricTrigger.metricResourceUri" | cut -d'/' -f9 | tr -d '"'`
