    ...
    for i in `seq 0 $count`; do
        ...
        if [ "$icount" -gt "0" ]; then
            ...
            for j in `seq 0 $icount`; do
                ...
                printf "\t name = \"%s\" \n"  $bname >> $prefix-$name.tf
                printf "\t port = \"%s\" \n"  $bport >> $prefix-$name.tf
                printf "\t protocol = \"%s\" \n"  $bproto >> $prefix-$name.tf
                printf "\t cookie_based_affinity = \"%s\" \n"  $bcook >> $prefix-$name.tf
                printf "\t request_timeout = \"%s\" \n"  $btimo >> $prefix-$name.tf
...
# proble block  probes=`echo $azr | jq ".[(${i})].probes"`
        ...
        if [ "$icount" -gt "0" ]; then
            ...
            for j in `seq 0 $icount`; do
                ...
                printf "\t name = \"%s\" \n"  $bname >> $prefix-$name.tf
                printf "\t protocol = \"%s\" \n"  $bproto >> $prefix-$name.tf
                printf "\t path = \"%s\" \n"  $bpath >> $prefix-$name.tf
                printf "\t host = \"%s\" \n"  $bhost >> $prefix-$name.tf
                printf "\t interval = \"%s\" \n"  $bint >> $prefix-$name.tf
                printf "\t timeout = \"%s\" \n"  $btimo >> $prefix-$name.tf
                printf "\t unhealthy_threshold = \"%s\" \n"  $bunth >> $prefix-$name.tf


                if [ "$bmsrv" != "null" ]; then
                printf "\t minimum_servers = \"%s\" \n"  $bmsrv >> $prefix-$name.tf
                fi
                ...
                if [ "$bmbod" != "null" ]; then
                    if [ "$bmbod" = "" ]; then
                        ...
                        printf "\t\t body = \"%s\" \n"  $bmbod >> $prefix-$name.tf
                    fi
                fi
                printf "\t }\n" >> $prefix-$name.tf
                
                #if [ "$bmstat" != "null" ]; then
                #printf "\t status_code = \"%s\" \n"  $bmstat >> $prefix-$name.tf
                #fi
