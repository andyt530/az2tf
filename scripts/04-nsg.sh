...
printf "resource \"azurerm_network_security_group\" \"%s\" { \n" $nsgnam > nsg-$nsgnam.tf
printf "\t name = \"%s\"  \n" $nsgnam >> nsg-$nsgnam.tf
printf "\t location = \"\${var.loctarget}\"\n"  >> nsg-$nsgnam.tf
printf "\t resource_group_name = \"\${var.rgtarget}\"\n" >> nsg-$nsgnam.tf
printf "}\n" >> nsg-$nsgnam.tf
terraform state rm azurerm_network_security_group.$nsgnam 
terraform import azurerm_network_security_group.$nsgnam $nsgid
done
